...
// Radio class
DRF1262 radio(spi0, SPIO_CS_RAD, SPIO_SCK, SPIO_POCI, SPIO_PICO, TXEN_PIN,
              DIO1_PIN, BUSY_PIN, SW_PIN, RST_PIN);
...
void init_pres(uint16_t *C1, uint16_t *C2, uint16_t *C3, uint16_t *C4,
               uint16_t *C5,
               uint16_t *C6) {  // Start I2C and retreive calibration constants
    // i2c initiate
    ...
    bi_decl(bi_2pins_with_func(
        SDA, SCL, GPIO_FUNC_I2C));  // Give binary info information about the
                                    // I2C idk what this actually does
    ...
    // Calibtation constant arrays. I2C is read in 8 bit chunks each going to an
    // array location to be concatenated later to create the full 16 bit number
    uint8_t C1t[2];
    uint8_t C2t[2];
    uint8_t C3t[2];
    uint8_t C4t[2];
    ...
    i2c_write_blocking(i2c1, addr_i2c, &C1_cmd, sizeof(C1_cmd),
                       false);  // Send command to ask for C1 vale
...
void read_pres(uint16_t C1, uint16_t C2, uint16_t C3, uint16_t C4, uint16_t C5,
               uint16_t C6, int32_t *TEMP,
               int32_t *P) {  // Takes Calibration constants and modifys presure
                              // and temperatuer variables
    // I2C commands
    uint8_t D1_cmd = 0x48;
    uint8_t D2_cmd = 0x58;
    uint8_t read = 0x00;
    ...
    i2c_write_blocking(i2c1, addr_i2c, &D1_cmd, sizeof(D1_cmd),
                       false);  // Write command for D1
    sleep_ms(100);              // Let sensor retreive
    i2c_write_blocking(i2c1, addr_i2c, &read, sizeof(read),
                       false);                         // Send read command
    i2c_read_blocking(i2c1, addr_i2c, D1t, 3, false);  // Read data
    D1 = 0x00000000 | (D1t[0] << 16) | (D1t[1] << 8) |
         D1t[2];  // Place 24 bit pressure value in 32 bit uint from 3 reac bits

    i2c_write_blocking(i2c1, addr_i2c, &D2_cmd, sizeof(D2_cmd),
                       false);  // Write command for D2
    sleep_ms(100);              // Let sensor retreive
    i2c_write_blocking(i2c1, addr_i2c, &read, sizeof(read),
                       false);                         // Send read command
    i2c_read_blocking(i2c1, addr_i2c, D2t, 3, false);  // Read data
    D2 = 0x00000000 | (D2t[0] << 16) | (D2t[1] << 8) |
         D2t[2];  // Place 24 bit pressure value in 32 bit uint from 3 reac bits
...
uint PWM_init() {  // Init PWM for motor control
    ...
    pwm_init(
        slice_num, &config,
        true);  // Start PWM for given slice number using given slice number
    pwm_set_chan_level(slice_num, 1, 0);  // Set PWM to 0
    return (slice_num);
}

void motor_speed_dir_set(bool dir,
                         int speed) {  // Set motor to specific speed and
                                       // direction. 0 is down 1 is up
    // PWM operates in a certian range with half of range as one direction other
    // half as other. Small zone in the middle for no movement
    if (speed == 0) {                             // If 0 have motor hold still
        ...
        pwm_set_chan_level(
            slice_num, 1,
            (12000 + (speed * 33)));  // Set PWM to scale of speed above center
    } else {
        pwm_set_chan_level(
            slice_num, 1,
            (11500 -
             (speed * 33)));  // Set PWM to scale of speed below the center
    }
}

void teather_dis(bool dir, int dis,
                 int speed) {  // move distance in direction at speed
    // 0 is down 1 is up
    int32_t start = get_count(
        &coder1, coder1_index);       // Retrieve starting position from encoder
    ...
    while (1) {
        ...
        if ((current >= (start + go)) ||
            (current <= (start - go))) {  // Compare current to offset of go in
                                          // either direction from start
            break;                        // exit loop
        }
    }
...
/*
void init_sd(FATFS fs, FIL fil, FRESULT *fr, const char* const
filename){//initalize SD card *fr = f_mount(&fs, "", 1);//mount SD card if
(FR_OK != *fr) panic("f_mount error: %s (%d)\n", FRESULT_str(*fr), *fr);//check
if it mounted correctly

    *fr = f_open(&fil, filename, FA_OPEN_APPEND | FA_WRITE);//Open file or
create if no file if (FR_OK != *fr && FR_EXIST != *fr)//check if file is open
correctly panic("f_open(%s) error: %s (%d)\n", filename, FRESULT_str(*fr), *fr);
...
void sd_write(FATFS fs, FIL fil, FRESULT *fr, const char* const filename,
int32_t P, int32_t TEMP){//Put information onto SD card *fr = f_mount(&fs, "",
1);//mount SD card if (FR_OK != *fr) panic("f_mount error: %s (%d)\n",
...
temperatuer and pressure to SD card and confirm write operation printf("f_printf
failed\n");
    }
    *fr = f_close(&fil);//Close file
